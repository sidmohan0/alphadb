...
STACK_NAME="${STACK_NAME:-alphadb-expensive-yes-live}"
SERVICE_NAME="${SERVICE_NAME:-alphadb-expensive-yes-live}"
REPOSITORY="${ECR_REPOSITORY:-alphadb-expensive-yes-live}"
IMAGE_TAG="${IMAGE_TAG:-$(git rev-parse --short HEAD)-$(date -u +%Y%m%d%H%M%S)}"
TEMPLATE_FILE="${TEMPLATE_FILE:-deploy/aws/expensive-yes-live-trading-job.yaml}"

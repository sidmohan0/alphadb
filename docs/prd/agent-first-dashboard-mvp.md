...
Humans need to commission and supervise autonomous strategy agents, not manually operate every trading loop. They need to express a strategy as a human-readable Strategy Brief and machine-readable Strategy Spec, observe what the agent is doing in the rhythm of the market, inspect the collected operational data, save useful data slices as reusable experiment inputs, and keep an experiment journal that accumulates agent and human learning over time.
...
4. Data Explorer: an evidence locker for browsing curated Postgres views, filtering records, exporting rows, and opening a filtered slice as a saved dataset snapshot.
5. Lab: an experiment journal for hypotheses, saved datasets, Strategy Spec snapshots, run summaries, notes, verdicts, and semantic experiment insights.
...
41. As a researcher, I want to open a filtered view as a saved dataset, so that an experiment can reference the exact data slice used.
42. As a researcher, I want saved dataset snapshots to include filters, row count, schema, query hash, and optional artifact location, so that the dataset remains reusable and traceable.
43. As a researcher, I want to see saved datasets in the Data Explorer sidebar, so that I can return to important slices.
44. As a researcher, I want to create a new experiment from a saved dataset, so that data exploration turns directly into research work.
45. As a Lab user, I want to create an experiment with a title and hypothesis, so that research starts with a claim we can test.
46. As a Lab user, I want to link an experiment to a Strategy Spec snapshot, so that the experiment knows what decision recipe was tested.
47. As a Lab user, I want to link an experiment to a saved dataset, so that the experiment knows which data slice was used.
48. As a Lab user, I want to attach replay, shadow, paper, or live run summaries, so that experiment results are gathered in one place.
49. As a Lab user, I want aggregate metrics such as trade count, win rate, P&L, and Sharpe-like summary where available, so that I can compare experiments.
50. As a Lab user, I want agent notes and human notes, so that interpretation is preserved alongside metrics.
51. As a Lab user, I want to mark an experiment as continue, revise, or kill, so that every experiment produces a decision.
52. As a Lab user, I want killed and revised experiments to stay visible, so that the agent can learn from failed branches.
53. As a Lab user, I want semantic insights that detect repeated patterns, warnings, similarities, and suggested next tests, so that experiment history becomes active memory.
54. As a Lab user, I want insights to link back to related experiments, so that I can inspect the evidence behind a suggestion.
55. As a Lab user, I want insight confidence to be visible, so that advisory analysis is not confused with proof.
56. As an agent, I want experiment verdicts and notes to be machine-readable, so that I can propose better next tests.
...
- Strategy Studio saves unsupported briefs as Research Ideas with compiler blockers and closest-template suggestions instead of pretending they are runnable specs.
...
- Data Explorer supports common filters, row limits, schema summaries, CSV/JSON export, and saved dataset snapshot creation.
- Saved dataset snapshots are persisted with enough provenance to recreate or explain the source query.
- Lab supports creating experiments, linking datasets and Strategy Spec snapshots, adding human and agent notes, attaching run summaries, recording metrics, and setting verdicts.
- Semantic experiment insights are persisted and displayed as pattern, warning, similarity, or suggestion records.
...
- If a Brief does not fit an existing Strategy Template, save it as a Research Idea in Lab with unsupported reasons, closest template suggestions, missing capabilities, and optional capability-request metadata.
- Persist Research Ideas and Experiments through one Lab Entry model with kind/status fields. Research Ideas hold uncompiled briefs and blockers; Experiments add dataset links, Strategy Spec snapshots, run summaries, metrics, notes, insights, and verdicts.
...
- Add Saved Dataset Snapshot persistence. A snapshot records source view, filters, row limit, sort, row count, column schema, query hash, creator label, and metadata. Exported CSV/JSON row artifacts are optional separate artifacts linked only when produced.
- Add an Experiment Journal. An experiment records title, hypothesis, status, verdict, linked Strategy Spec snapshot, linked saved dataset snapshot, configuration summary, aggregate metrics, human notes, agent notes, and timestamps.
- Add Experiment Run Summary records. These connect an experiment to replay, shadow, paper, gated-live, or live run ids and store lightweight metrics from those runs.
- Add Semantic Experiment Insight records. Insights should store type, text, related experiment ids, related dataset ids when applicable, confidence, source, generated timestamp, and status.
...
- Preserve provenance automatically. Dataset snapshots, experiment records, notes, run summaries, and insights should be saved by default because memory is part of the product, not an optional compliance add-on.
...
- Add repository tests for Saved Dataset Snapshots. Tests should verify snapshot creation, deterministic query hashing, row-count capture, list ordering, and retrieval by id.
- Add repository tests for Experiment Journal records. Tests should verify experiment creation, Strategy Spec snapshot persistence, dataset links, notes, verdict changes, run summary links, and aggregate metrics storage.
- Add tests for Semantic Experiment Insight generation. Tests should verify that repeated tags, similar hypotheses, and negative verdict patterns can produce stable advisory insight records without relying on an external LLM.
...
The supplied design prototype already contains strong first-pass implementations of the shell, Strategy Studio, Live Operations components, Agent Terminal, skill registry, mock data, and settings screens. The MVP should copy and adapt that work rather than recreating it from scratch.

The two new screens provided by the user should be treated as target UX for Data Explorer and Lab:

- Data: "Evidence locker - browse, filter, export, snapshot."
- Lab: "Experiment journal - hypotheses, verdicts, semantic memory."

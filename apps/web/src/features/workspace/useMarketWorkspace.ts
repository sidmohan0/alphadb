...
import {
  ...
  displayMarketsForMode,
  loadLocalPersistentState,
  loadStoredToken,
  persistLocalState,
  persistToken,
  PROVIDERS,
  ...
  type ViewMode,
  type WorkspaceState,
  updatePersistentStateMarkets,
} from "./shared";

export interface MarketWorkspaceController {
  apiTokenDraft: string;
  setApiTokenDraft: (value: string) => void;
  applyApiToken: () => void;
  authStatus: AuthStatus | null;
  backendStateEnabled: boolean;
  query: string;
  ...
  setFocusedProvider: (provider: ProviderId) => void;
  ...
  providerState: Record<ProviderId, ProviderSelectionState>;
  displayedMarkets: Record<ProviderId, MarketSummary[]>;
  selectedMarkets: Record<ProviderId, MarketSummary | null>;
  savedIds: Set<string>;
  recentIds: Set<string>;
  selectMarket: (provider: ProviderId, index: number) => void;
  toggleSaveFocusedMarket: () => void;
  refreshAll: () => void;
}

export function useMarketWorkspace(): MarketWorkspaceController {
  ...
  const [focusedProvider, setFocusedProvider] = useState<ProviderId>("polymarket");
  ...
  const [providerState, setProviderState] = useState<Record<ProviderId, ProviderSelectionState>>({
  ...
  const savedIds = useMemo(
    () => new Set(workspace.persistent.savedMarkets.map((entry) => entry.market.id)),
    [workspace.persistent.savedMarkets],
  );
  const recentIds = useMemo(
    () => new Set(workspace.persistent.recentMarkets.map((entry) => entry.market.id)),
    [workspace.persistent.recentMarkets],
  );
  ...
  useEffect(() => {
    ...
    const stream = client.createMarketStream({
      ...
      onUpdate: (update: MarketStreamUpdate) => {
        startTransition(() => {
          setWorkspace((current) => ({
            trending: {
  ...
  const selectMarket = (provider: ProviderId, index: number): void => {
  ...
  const toggleSaveFocusedMarket = (): void => {
    const market = selectedMarkets[focusedProvider];
    if (!market) {
      return;
    }
